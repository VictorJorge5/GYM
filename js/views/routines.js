    ...
    <section class="section">
      <div class="section__head">
        ...
        <div style="display:flex;gap:8px;">
          <a class="btn btn--ghost btn--small" href="#/routines/generate">🤖 Generar</a>
...
export async function renderEditor(container, params) {
  ...
  function exFor(item) {
    return allExercises.find((e) => e.id === item.exerciseId);
  }
  ...
  function renderList() {
      ...
      ? routine.exercises
        ...
        <li class="editor-exercise" data-i="${i}">
          <div class="editor-exercise__head">
            <button type="button" class="exercise-row-name" data-view="${i}">
              <img src="${mediaUrl(exFor(item)?.image)}" class="exercise-thumb" alt="" loading="lazy">
              <span>${escapeHtml(item.name)}</span>
            </button>
            <div class="editor-exercise__move">
              <button class="icon-btn" data-swap="${i}" aria-label="Cambiar ejercicio" title="Cambiar por otro similar">🔄</button>
    ...
    listEl.querySelectorAll('[data-view]').forEach((b) =>
      (b.onclick = () => openExerciseDetail(exFor(routine.exercises[Number(b.dataset.view)])))
    );
    listEl.querySelectorAll('[data-swap]').forEach((b) =>
      (b.onclick = () => {
        const i = Number(b.dataset.swap);
        openSwapPicker(allExercises, routine.exercises[i].exerciseId, (newEx) => {
          routine.exercises[i] = { ...routine.exercises[i], exerciseId: newEx.id, name: newEx.name, equipment: newEx.equipment };
          renderList();
        });
      })
    );
