  ...
  function renderDrafts() {
      ...
      <div class="editor-exercise" style="margin-bottom:16px;">
        ...
        ${d.exercises
          ...
          <div class="editor-exercise" style="background:var(--surface-2);">
            <div class="editor-exercise__head">
              <button type="button" class="exercise-row-name" data-view="${di}:${ei}">
                <img src="${mediaUrl(exFor(ex).image)}" class="exercise-thumb" alt="" loading="lazy">
    ...
    listEl.querySelectorAll('[data-view]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const [di, ei] = btn.dataset.view.split(':').map(Number);
        openExerciseDetail(exFor(drafts[di].exercises[ei]));
      });
    });
